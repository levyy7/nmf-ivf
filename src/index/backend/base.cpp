...
void IVFBackend::build(const SpMat& X, Eigen::MatrixXf H) {
    // Sink idiom: Move the passed Matrix directly into the member.
...
std::vector<std::vector<IVFBackend::SearchResult>> IVFBackend::search(const SpMat& queries, const SpMat& X_docs,
    int top_k, const SearchParams* params) const
{
    ...
    for (int qi = 0; qi < Q; qi += CHUNK) {
        ...
        for (int i = 0; i < chunk_size; ++i) {
            ...
            results[qi + i] = search_one(query, chunk_projected.row(i), X_docs, top_k, params);
        }
    }
