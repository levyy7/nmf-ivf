...
NMFBase::Init parseInitMethod(const std::string& s)
{
    if (s == "acol")   return NMFBase::Init::Acol;
    if (s == "random") return NMFBase::Init::Random;
    throw std::invalid_argument("Unknown init method: '" + s +
                                "'. Valid: acol, random");
}
...
int main(int argc, char** argv)
{
    ...
    nmf->add_option("--nmf",          cfg.nmf_type,    "NMF solver: hals | mu")
        ->capture_default_str();
    ...
    nmf->add_option("--init",         cfg.init_method,
        "Initialisation: acol (default) | random")
        ->capture_default_str();
    nmf->add_option("--acol-p",       cfg.acol_p,
    ...
    // Apply preset first, then explicit overrides
    ...
    base.debug = app.count("--debug") > 0;  // flags don't need preset merging
    cfg = base;

    // Resolve seed: explicit value if set, otherwise a fresh one from the OS
    const unsigned int seed = cfg.random_state.has_value()
        ? static_cast<unsigned int>(*cfg.random_state)
        : std::random_device{}();


    // Parse init method string → enum once, fail early with a clear message
