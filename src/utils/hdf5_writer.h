...
public:
    ...
    // ────────────────────────────────────────────────────────────────
    // Opens an existing HDF5 file (to append/overwrite datasets) or creates a new one.
    explicit HDF5Writer(const std::string& filepath) {
        ...
        if (file_ < 0) {
            file_ = H5Fcreate(filepath.c_str(), H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT);
            if (file_ < 0) {
                throw std::runtime_error("Failed to open or create HDF5 file: " + filepath);
            }
        }
    }
    ...
    // ── Write knns (n x k integers) ─────────────────────────────────
    void writeKnns(const Eigen::MatrixXi& knns, bool is_zero_based = true) {
        if (is_zero_based) {
            Eigen::MatrixXi knns_1based = knns.array() + 1; // Padded -1s will safely become 0s
    ...
    void writeIndex(const Eigen::MatrixXf& H, const Eigen::MatrixXi& lists) {
        ...
        H5Handle grp{ H5Gcreate2(file_, "index", H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT), H5Gclose };
        if (grp.id < 0) {
    ...
    void writeAttribute(const std::string& attr_name, const std::string& value) {
        ...
        hid_t attr = H5Acreate2(root, attr_name.c_str(), type, space, H5P_DEFAULT, H5P_DEFAULT);
        if (attr >= 0) {
    ...
    void writeAttribute(const std::string& attr_name, float value) {
        ...
        hid_t space = H5Screate(H5S_SCALAR);
        hid_t attr = H5Acreate2(root, attr_name.c_str(), H5T_NATIVE_FLOAT, space, H5P_DEFAULT, H5P_DEFAULT);
        if (attr >= 0) {
...
private:
    ...
    void writeDenseDataset(hid_t loc_id, const std::string& name, const Eigen::MatrixBase<Derived>& mat) {
        ...
        Eigen::Matrix<Scalar, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> row_major_mat = mat;
        ...
        if (space.id < 0) throw std::runtime_error("Failed to create dataspace for " + name);
        ...
        herr_t status = H5Dwrite(ds.id, h5Type<Scalar>(), H5S_ALL, H5S_ALL, H5P_DEFAULT, row_major_mat.data());
        if (status < 0) throw std::runtime_error("Failed to write data to dataset " + name);
    }
