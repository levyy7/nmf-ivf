...
inline double mean(const std::vector<double>& v) {
  ...
  return std::accumulate(v.begin(), v.end(), 0.0) / static_cast<double>(v.
           size());
}
...
void evaluate_nmf_index(
    ...
    const std::vector<std::pair<std::string, const IVFBackend::SearchParams*>>&
    param_sweep
    ) {
  ...
  double total_sec = std::chrono::duration<double>(global_t1 - global_t0).
      count();
  double avg_query_ms = (total_sec * 1000.0) / static_cast<double>(queries.
                          rows());
